...
public:
	...
	*/
	bool distinct(Point& p) const { return BasicGeometry::equal(_x, p._x) or BasicGeometry::equal(_y, p._y); }

	/**
	*	@brief Checks if the coordinates of this point are equal from the coordinates of a point p.
	*/
	bool equal(Point& p) const { return BasicGeometry::equal(_x, p._x) and BasicGeometry::equal(_y, p._y); }
